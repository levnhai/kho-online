...
export function getOrderStatusColor(status: string): {
  ...
  switch (status) {
    ...
    case "PARTIAL_DELIVERED":
      return {
        bg: "bg-amber-100/90 dark:bg-amber-950/80",
...
export function getDeliveryBatchStatusText(status?: string): string {
  switch (status) {
    case "PREPARING":
...
export function getDeliveryBatchStatusColor(status?: string): {
  bg: string;
  text: string;
  border: string;
} {
