...
export const AdminDashboardPage: React.FC = () => {
  ...
  const fetchDashboardData = useCallback(async (rangeToFetch: TimeRange = selectedRange, isSilent = false) => {
  ...
  if (error && !data) {
    const isUnauthorized = error.toLowerCase().includes('unauthorized') || error.toLowerCase().includes('tài khoản');
    return (
      <div className="p-6 text-center bg-white dark:bg-slate-800 rounded-2xl border border-rose-150 dark:border-rose-900/40 shadow-sm space-y-3 max-w-md mx-auto my-8">
        ...
        <div>
          <h3 className="text-sm font-bold text-gray-900 dark:text-white">
            {isUnauthorized ? 'Phiên đăng nhập đã hết hạn' : 'Lỗi nạp dữ liệu Dashboard'}
          </h3>
        ...
        <div className="flex items-center justify-center gap-2 pt-1">
          {isUnauthorized ? (
            ...
            <Button variant="primary" size="sm" onClick={() => fetchDashboardData(selectedRange)} className="text-xs py-1.5 px-3">
              Thử lại
            </Button>
          )}
        </div>
  ...
  const { metrics, topSelling, recentOrders, revenueChart, rangeLabel } = data || {
    rangeLabel: '7 ngày qua',
    metrics: { totalProducts: 0, totalOrders: 0, allTimeOrders: 0, totalCustomers: 0, allTimeCustomers: 0, totalRevenue: 0, allTimeRevenue: 0 },
    topSelling: [],
    recentOrders: [],
    revenueChart: [],
  };

  const maxRevenue = Math.max(...(revenueChart?.map((c) => c.revenue) || [1]), 1);
    ...
    <div className="space-y-4 text-gray-900 dark:text-white pb-4">
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-white dark:bg-slate-800 p-3 sm:p-4 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-xs">
        <div className="flex items-center justify-between sm:justify-start gap-2.5">
          ...
          <span className="text-xs text-gray-400 dark:text-slate-500 hidden md:inline">
            Dữ liệu theo: <strong className="text-gray-700 dark:text-slate-300 font-semibold">{rangeLabel || '7 ngày qua'}</strong>
          </span>
        </div>
        ...
        <div className="flex items-center justify-between sm:justify-end gap-2">
          ...
          >
            ...
            <span className="hidden sm:inline">{refreshing ? 'Cập nhật...' : 'Làm mới'}</span>
          </button>
        </div>
      </div>
        ...
        >
          <div>
            ...
            <span className="text-[10px] text-blue-600 dark:text-blue-400 font-semibold mt-1 inline-flex items-center gap-0.5">
              Đang bán
              <ArrowRight size={9} className="opacity-0 group-hover:opacity-100 -translate-x-1 group-hover:translate-x-0 transition-all" />
            </span>
          </div>
        ...
        >
          <div>
            ...
            <span className="text-[10px] text-emerald-600 dark:text-emerald-400 font-semibold mt-1 inline-flex items-center gap-0.5">
              {metrics.allTimeOrders ? `Tổng: ${metrics.allTimeOrders} đơn` : 'Xem tất cả'}
              <ArrowRight size={9} className="opacity-0 group-hover:opacity-100 -translate-x-1 group-hover:translate-x-0 transition-all" />
            </span>
          </div>
        ...
        >
          <div>
            ...
            <span className="text-[10px] text-amber-600 dark:text-amber-400 font-semibold mt-1 inline-flex items-center gap-0.5">
              {metrics.allTimeRevenue ? `Tổng: ${formatCurrency(metrics.allTimeRevenue)}` : 'Chi tiết'}
              <ArrowRight size={9} className="opacity-0 group-hover:opacity-100 -translate-x-1 group-hover:translate-x-0 transition-all" />
            </span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-3 sm:gap-4 items-start">
        ...
        <div className="lg:col-span-7 bg-white dark:bg-slate-800 p-4 sm:p-5 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-xs space-y-3 transition-colors">
          <div className="flex items-center justify-between pb-2 border-b border-gray-100 dark:border-slate-700">
            ...
            <Link to="/admin/statistics" className="text-[10px] sm:text-xs font-bold text-blue-600 dark:text-blue-400 hover:underline">
          ...
          {revenueChart && revenueChart.length > 0 ? (
            <div className="h-40 sm:h-48 flex items-end justify-between gap-1 sm:gap-2.5 pt-6 px-1">
              {revenueChart.map((item, i) => {
                const heightPercent = item.revenue > 0 ? Math.max(14, Math.round((item.revenue / maxRevenue) * 100)) : 6;
                return (
                  <div key={i} className="flex-1 flex flex-col items-center h-full justify-end group relative cursor-pointer">
                    ...
                    <div className="absolute -top-11 opacity-0 group-hover:opacity-100 transition-all transform group-hover:-translate-y-1 bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 text-[10px] font-bold py-1 px-2 rounded-lg pointer-events-none whitespace-nowrap z-20 shadow-lg flex flex-col items-center border border-slate-700 dark:border-slate-200">
                      <span className="text-amber-300 dark:text-amber-700">{item.date}</span>
                      <span className="font-extrabold">{formatCurrency(item.revenue)}</span>
                      <span className="text-[9px] font-normal text-slate-300 dark:text-slate-600">{item.orders} đơn hàng</span>
                    </div>
                    {/* Bar */}
                    <div
        ...
        <div className="lg:col-span-5 bg-white dark:bg-slate-800 p-4 sm:p-5 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-xs space-y-2.5 transition-colors">
          <div className="flex items-center justify-between pb-2 border-b border-gray-100 dark:border-slate-700">
            <h3 className="font-bold text-gray-900 dark:text-white text-xs sm:text-sm">TOP BÁN CHẠY</h3>
            <span className="text-[10px] text-gray-400 dark:text-slate-400">Số lượng bán</span>
          </div>
          ...
          {topSelling && topSelling.length > 0 ? (
            <div className="divide-y divide-gray-100 dark:divide-slate-700 space-y-0.5">
              {topSelling.map((sp: any, idx: number) => {
                const medals = ['🥇', '🥈', '🥉', '4.', '5.'];
                return (
                  <div key={sp._id} className="pt-2 pb-1.5 flex items-center justify-between gap-2 text-xs hover:bg-gray-50/50 dark:hover:bg-slate-750 rounded-lg px-1 transition-colors">
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="font-semibold text-gray-900 dark:text-white truncate text-[11px]" title={sp.name}>
                          {sp.name}
                        </p>
      ...
      <div className="bg-white dark:bg-slate-800 p-4 sm:p-5 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-xs space-y-3 transition-colors">
        <div className="flex items-center justify-between pb-2 border-b border-gray-100 dark:border-slate-700">
          <div className="flex items-center gap-1.5">
            ...
            <h3 className="font-bold text-gray-900 dark:text-white text-xs sm:text-sm">ĐƠN HÀNG VỪA ĐẶT</h3>
          </div>
          <Link to="/admin/orders" className="text-[11px] font-bold text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1">
              ...
              <table className="min-w-full text-left text-xs">
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-slate-700">
                  {recentOrders.map((ord: any) => {
                    ...
                    const customerName = ord.customerInfo?.name || (typeof ord.customer === 'object' ? ord.customer?.name : 'Khách vãng lai');
                    return (
                      <tr key={ord._id} className="hover:bg-gray-50/50 dark:hover:bg-slate-700/30 transition-colors">
                        <td className="py-2.5 px-3 font-bold text-blue-600 dark:text-blue-400 whitespace-nowrap text-xs">{ord.orderCode}</td>
                        ...
                        <td className="py-2.5 px-3 whitespace-nowrap">
                          <span className={`px-2 py-0.5 rounded-full font-bold text-[9px] border ${color.bg} ${color.text} ${color.border}`}>
