...
async function sync() {
  try {
    ...
    // 1. Sync Categories with subcategories
    ...
    for (const cat of initialCategories) {
      ...
      if (existing) {
        await db.collection('categories').updateOne(
          { _id: existing._id },
          { $set: { subcategories: cat.subcategories, name: cat.name, description: cat.description, icon: cat.icon, updatedAt: new Date() } }
        );
    ...
    // 2. Check if Users exist, if not seed
    ...
    // 3. Check if products exist, if not copy or seed initial products
